...
def build_generator():
    ...
    cl = Embedding(10, 100, embeddings_initializer = 'glorot_normal')(image_class)
...
def plot_gen(generator, i):
    for j in range(10):
        ...
        generated_image = generator.predict([noise, random_label])
        plt.figure()
...
def train(X, Y, generator, discriminator, GAN, epochs = 1000, batch_size = 32):
    ...
    for i in tqdm(range(epochs)):
        ...
        generated_images = generator.predict([noise, random_labels.reshape(-1, 1)])
        
        X_train = np.concatenate((real_images, generated_images))
        real_or_fake = np.array([1] * batch_size + [0] * batch_size)
        random_labels = keras.utils.to_categorical(random_labels, 10)
        ...
        g_loss_total, g_loss_source, g_loss_class = GAN.train_on_batch([noise, random_labels], [real_or_fake, class_labels])
